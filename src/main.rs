...
fn main() -> Result<()> {
    ...
    for &(repo, subfolder) in &REPOSITORIES {
        match fetch_artifact(&client, repo, build_type) {
            Ok(artifact) => {
                if let Err(e) = download_and_extract_artifact(&client, repo, &artifact, &final_path, subfolder) {
                    eprintln!("{}", format!("Error downloading/extracting artifact: {}", e).red());
                }
            }
            Err(e) => eprintln!("{}", format!("Error fetching artifact for {}: {}", repo, e).red()),
        }
    }
    ...
    println!("{}", "https://github.com/NVIDIAGameWorks/rtx-remix/wiki/runtime-user-guide".cyan());

    Ok(())
}

fn fetch_artifact(client: &Client, repo: &str, build_type: &str) -> Result<(String, u64)> {
    println!("{}", format!("Fetching artifact for {} ({})", repo, build_type).cyan());
    ...
    let artifact = artifacts["artifacts"]
        ...
        .and_then(|artifacts| {
            artifacts.iter().find(|a| {
                a["name"].as_str().map_or(false, |name| name.contains(build_type))
            })
        })
    ...
    println!("{}", format!("Found artifact: {} (ID: {})", artifact_name, artifact_id).green());

    Ok((artifact_name, artifact_id))
}

fn download_and_extract_artifact(client: &Client, repo: &str, artifact: &(String, u64), final_path: &Path, subfolder: &str) -> Result<()> {
    let (artifact_name, artifact_id) = artifact;
    let download_url = format!("https://nightly.link/{}/actions/artifacts/{}.zip", repo, artifact_id);

    println!("{}", format!("Downloading artifact: {}", artifact_name).cyan());
    ...
    let path = final_path.join(subfolder).join(format!("{}.zip", artifact_name));
    ...
    println!("{}", format!("Extracting artifact: {}", artifact_name).cyan());
...
fn cleanup_debug_files(dir: &Path) -> Result<()> {
    ...
    if removed_files > 0 {
        println!("{}", format!("Cleaned up {} debugging symbol files", removed_files).cyan());
    }
    Ok(())
}
...
fn cleanup_debug_files_recursive(dir: &Path, removed_files: &mut u32) -> Result<()> {
    for entry in fs::read_dir(dir)? {
        ...
        } else if path.file_name().unwrap() == "CRC.txt" || path.file_name().unwrap() == "artifacts_readme.txt" {
            fs::remove_file(&path)?;
            *removed_files += 1;
        }
    }
...
fn download_and_extract_dx8_binaries(client: &Client, final_path: &Path) -> Result<()> {
    ...
    let dx8_url = "https://nightly.link/elishacloud/dxwrapper/workflows/ci/master/dx8%20binaries.zip";
...
fn clickable_path(path: &Path) -> String {
    ...
    format!("\x1B]8;;file://{}\x07{}\x1B]8;;\x07", clean_path, clean_path).cyan().to_string()
}